...
# 2. 核心特性
(1)配置化采集：通过JSON配置定义选择器规则，90%的网站无需写代码
(2)智能降级：多选择器自动回退机制，提高数据提取成功率
(3)统一数据模型：标准化的新闻数据结构，便于后续处理
(4)多格式日期解析：自动识别并解析20+种常见日期格式
(5)数据存储：推荐存储在 MongoDB
(6)易于扩展：新增网站只需配置，平均10分钟完成适配
